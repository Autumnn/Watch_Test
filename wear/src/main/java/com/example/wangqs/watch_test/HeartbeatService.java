...
public class HeartbeatService extends Service implements SensorEventListener {
    ...
    public class HeartbeatServiceBinder extends Binder {
        public void setChangeListener(OnChangeListener listener) {
            ...
            listener.onValueChanged(currentValue_Y, currentValue_Z);
        }
...
//        Sensor mHeartRateSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_HEART_RATE);
        Sensor mHeartRateSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
...
//            int newValue = Math.round(sensorEvent.values[0]);
            int newValue_Y = Math.round(sensorEvent.values[1]*100);
            int newValue_Z = Math.round(sensorEvent.values[2]*100);
            ...
            if(currentValue_Y != newValue_Y && newValue_Y!=0) {
                Y_changed = true;
                currentValue_Y = newValue_Y;                    // save the new value
            }

            if(currentValue_Z != newValue_Z && newValue_Z!=0){
                Z_Changed = true;
                currentValue_Y = newValue_Z;                    // save the new value
            }
                // send the value to the listener
            if(Y_changed||Z_Changed) {
                if (onChangeListener != null) {
                    Log.d(LOG_TAG, "sending new value to listener: " + newValue_Y + newValue_Z);
                    onChangeListener.onValueChanged(newValue_Y, newValue_Z);
                    String Value = Integer.toString(newValue_Y) + "-" + Integer.toString(newValue_Z);
                    sendMessageToHandheld(Value);
                }
                Y_changed = false;
                Z_Changed = false;
            }
